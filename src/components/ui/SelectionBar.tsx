...
interface Props {
  ...
  onClear: () => void;
  deleting?: boolean;
  /** Singular noun, e.g. "budget" → "1 budget selected" / "3 budgets selected". */
  noun?: string;
}

/** Selection action bar shown above a list/grid when one or more rows are selected. */
export default function SelectionBar({ count, onDelete, onClear, deleting = false, noun = "item" }: Props) {
